"""Bear Call Spread signal engine for vibe-trading-ai's
options_portfolio.run_options_backtest.

Entry rule: shared with scripts/signal_core.py — do NOT reimplement here;
change signal_core.py so backtest and signal definitions stay in lockstep.

On a confirmed bear entry (and no open spread on that ticker): open a $10-wide
Bear Call Spread, short strike = MA150 rounded up to nearest $5 (above
resistance), long strike = short + 10, expiry = next Friday ~30
...
class BearCallSpreadSignalEngine:
    ...
    def generate(self, data_map: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for code, df in data_map.items():
            ...
            for i in range(len(df)):
                ...
                # bear_entry_checks inspects bars[-2:]; give it signal bar i and
                ...
                signals.append({
                    ...
                    "legs": [
                        {"type": "call", "strike": short_strike, "expiry": expiry_str, "qty": -1},
                        {"type": "call", "strike": long_strike, "expiry": expiry_str, "qty": 1},
                    ],
