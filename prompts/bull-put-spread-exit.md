...
6. **Classify position verdict:**
   **Design principle — judgment may escalate, never suppress.** Hard-CLOSE triggers are deterministic and must never be downgraded, overridden, or suppressed by any judgment check. RECOMMEND EXIT is an escalation tier evaluated when no hard-CLOSE trigger fired, or to surface genuine judgment calls.
   ...
   - 🔴 **CLOSE (hard, unconditional, deterministic):**
     - `thesis_invalidated` is true (`close < short_strike` OR `close < MA150` — both are hard-CLOSE conditions as of 2026-09-05; see `MA150_BREACH` reason code below), OR
     ...
     - **Reason code on thesis invalidation:** when `thesis_invalidated` fires, name which condition(s) tripped it:
       - Strike only: `STRIKE_BREACH: close ($<close>) below short strike ($<short_strike>)`.
       - MA150 only: `MA150_BREACH: close ($<close>) below 150-day MA ($<ma150>), short strike ($<short_strike>) not yet breached. Reinstated as hard CLOSE 2026-09-05 (briefly advisory-only earlier that day) — a $3k Global-Heap-Allocator backtest on the expanded 1,591-ticker universe found uniform MA150-as-a-hard-stop outperforming strike-only by 5.7x monthly P&L, including on the mega-cap subset alone, contradicting the earlier single-universe result. Shipped for forward-testing; contradiction intentionally left for live data to resolve.`
       - Both: `STRIKE_AND_MA150_BREACH: close ($<close>) below both short strike ($<short_strike>) and 150-day MA ($<ma150>)`.
     - **Annotate, never suppress on thesis invalidation:** check `checks.volume_confirmed_breakdown`. If false (breach occurred on below-average volume), the verdict is STILL CLOSE (hard trigger is never weakened) — but append `(light volume — possible whipsaw, verify manually)` to the reason. If volume confirmed the breakdown, no annotation needed, or optionally note `(volume-confirmed breakdown)`.
