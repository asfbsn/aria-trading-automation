...
**PRIORITIZATION & ALLOCATION — "The Global Heap" (dynamic malloc model, updated 2026-09-02):** portfolio risk is managed as one shared pool, not fixed per-trade % or a fixed position-count cap — replaces the prior "High-Conviction/Velocity" 25%/2-position split (same 50% total ceiling, allocation mechanics changed).

- **The Heap (unbreachable):** total concurrent exposure across ALL open bull-put-spread positions is hard-capped at **50% of net_liq** — unchanged from every sizing model used this session. Call `get_account_summary` (net_liq) and `get_account_positions` (open validated spreads — same leg-pair validation as before: same underlying/expiration, both PUTS, offsetting quantities, short strike ABOVE long strike). `heap_remaining = 0.50 × net_liq − Σ (over open validated spreads) of (width − credit_received) × 100 × contracts`. If `credit_received` isn't recoverable from `get_account_positions`, over-reserve conservatively using `width × 100 × contracts` instead (worst-case max loss — never under-reserve). **Unpaired or ambiguous legs (a naked short put sitting alone, a partial fill) are a real, unhedged risk, not an exclusion:** flag them "verify manually" as before, AND `BLOCKED — unpaired position, resolve before new allocation` for the ENTIRE scan — do not run the allocator or emit any new executable directive while one exists. A naked short's worst case (strike × 100 × contracts) isn't a number the heap accounting can safely net against a defined-risk spread's max loss, so the safe response is to halt new risk-taking until a human resolves it, not to estimate around it. **Freeing capital is automatic, no separate bookkeeping:** a position that hit its GTC profit-take or was closed by `exit-guard.sh` simply no longer appears in `get_account_positions` on the next scan, so its capital is already back in `heap_remaining`.
- **Ranking (this scan's PRIME survivors only — not a lookback across days; typically 0–3 names, so this is a light tiebreak layer, not a state machine):** rank by R/R at each candidate's PRIMARY (widest-clearing) width, **ascending** — lowest R/R first. Lower R/R means more credit collected per dollar of risk (R/R = width/credit − 1, so ascending R/R = descending credit/width — same ordering, stated both ways so this can't be inverted). The richest-priced setup gets first claim on the heap.
- **Dynamic block size per candidate (no static per-trade % or position-count cap; this is the "malloc"):** walking the ranked list top-down:
  1. Try the candidate's PRIMARY width. If its 1-contract max_loss (using the floor credit, width/3.5 — see HOW MUCH) exceeds `min(0.25 × net_liq, heap_remaining)` — checking against the per-trade ceiling too, not `heap_remaining` alone, so a width that would round down to 0 contracts under the per-trade cap is never selected — fall back through R/R VERIFICATION's cleared-widths list **from widest to narrowest** (R/R VERIFICATION *records* that list narrowest-first for display, but the allocator walks it in the opposite direction: try the next-widest cleared width below PRIMARY before ever dropping to a narrower one, since a wider fit keeps more credit relative to commissions and the $0.30 liquidity gate than jumping straight to the narrowest option). If NONE of its cleared widths fit `min(0.25 × net_liq, heap_remaining)` → `BLOCKED — heap exhausted` (distinct from BLOCKED-too-wide: the signal is real and would size fine with more capital, it's simply unavailable this round because higher-ranked candidates already claimed it).
  2. Once a width fits: `contracts = floor( min(0.25 × net_liq, heap_remaining) / max_loss_per_contract )`, at least 1 if it fits at all. **The `0.25 × net_liq` figure is a per-trade ceiling, carried over as an interpretive choice from the block-size examples in the request** (a $10-wide spread naturally costs ~20–30% of a $3k account, a $5-wide ~15% — this stops a single name from swallowing the entire heap even when it's the only or richest candidate that day; not explicitly requested, flagged here so it can be removed if a single name should be allowed the full 50%).
  3. Deduct `max_loss_per_contract × contracts` from `heap_remaining`, move to the next-ranked candidate, repeat until `heap_remaining` can't fit any remaining candidate's narrowest cleared width, or candidates run out.
- **Sector guard unchanged:** a candidate whose sector (from `data/universe.csv`) matches an already-open position, OR a higher-ranked candidate already allocated earlier in this same round, → `BLOCKED — sector correlation with <ticker>` (checked before allocation, same rule as before — just now also checked against this round's own higher-ranked picks, not only prior days' open positions).
- **Tail risk, stated once:** position count now floats by geometry (typically 2–5 names, depending on width/price) instead of a fixed cap. Total worst-case exposure is unchanged at 50% of net_liq. Single-name worst case is bounded by the 25%-of-net_liq per-trade ceiling above; two correlated breaches (a broad-market drawdown hitting multiple open names at once — the same scenario class the bear-track kill was grounded in) still costs up to 50% in one stroke. That risk doesn't disappear under this model, it's just sized per-trade by geometry instead of a blunt fixed %.
...
- **EXITS (mandatory in every executable block):**
  ...
  - Stop: close if the underlying CLOSES below the short strike (`thesis_invalidated` — the only hard-CLOSE price trigger, identical to `prompts/bull-put-spread-exit.md`'s exit guard). A close below MA150 alone is NOT a hard stop — it's the advisory `MA150_BREACH` RECOMMEND EXIT flag (locked 2026-09-05, final config: a Global-Heap-Allocator backtest found MA150-as-hard-stop catastrophic when paired with the loosened entry rule), monitored daily by `exit-guard.sh`. Human judgment call, not an automatic close.
