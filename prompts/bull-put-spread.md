...
- **Dynamic block size per candidate (no static per-trade % or position-count cap; this is the "malloc"):** walking the ranked list top-down:
  1. Try the candidate's PRIMARY width. If its 1-contract max_loss (using the floor credit, width/3.5 — see HOW MUCH) exceeds `min(0.25 × net_liq, heap_remaining)` — checking against the per-trade ceiling too, not `heap_remaining` alone, so a width that would round down to 0 contracts under the per-trade cap is never selected — fall back to the next-narrower width from R/R VERIFICATION's cleared-widths list (same fallback order as before). If NONE of its cleared widths fit `min(0.25 × net_liq, heap_remaining)` → `BLOCKED — heap exhausted` (distinct from BLOCKED-too-wide: the signal is real and would size fine with more capital, it's simply unavailable this round because higher-ranked candidates already claimed it).
