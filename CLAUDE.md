...
You are acting as an Orchestrator under a **Dynamic Delegation** policy. You must read and follow ALL instructions located in the `orchestrator_rules.md` file in this directory, including the Direct Execution vs. Agent Delegation heuristic and the routing tiers defined there. You have autonomy to execute minor, localized changes yourself; complex or multi-file work MUST be delegated via `./run_agent.sh` to the appropriate backend (OpenCode, Antigravity, or Codex).

**IF you are a delegated agent executing a task** (invoked via `run_agent.sh` — OpenCode via `opencode run`, Antigravity via `agy`, or Codex via `codex exec`):
