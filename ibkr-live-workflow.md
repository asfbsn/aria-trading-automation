...
6. **Size discipline (updated 2026-09-02).** Spread width is dynamic — derived from each name's actual live chain spacing (S, 2S, 4S), not a fixed $10; the directive lists a PRIMARY (widest-clearing) width and any narrower FALLBACK widths. Max loss per spread = **10% of current net liquidation value** (`get_account_summary`); contracts = floor(10% × net_liq / ((width − credit) × 100)), tried at the PRIMARY width first, then narrower fallbacks if that sizes to 0; a result of 0 at every tested width means the spread is too wide for the account — do not stage. **Max 5 concurrent spreads** (10% × 5 = same ≈50% total portfolio risk ceiling as before) and **one spread per sector** (sector from `data/universe.csv`). The nightly scan's TRADE DIRECTIVE block computes all of this — a staged ticket should match its numbers (including which width) or state why it deviates.
...
1. **I confirm size + price against the directive:** quantity from the 10% net-liq formula (golden rule 6) at the directive's actual chosen width, and net-credit limit from the directive's verified mid for that width (floor: width/3.5, the 1:2.5 boundary). If you want to deviate from the directive's numbers, say so explicitly — I never silently substitute my own.
